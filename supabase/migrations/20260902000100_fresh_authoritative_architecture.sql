...
-- Backward-compatible on a fresh database. Existing data is not imported automatically.
...
create table if not exists public.ingredients (
  ...
  administration jsonb default '{}'::jsonb,
...
create table if not exists public.neml_memberships (
  ...
  strength text,
  category text,
  edition text not null default '7th Edition 2020',
  source_id uuid references public.drug_sources(id) on delete set null,
  evidence text,
  created_at timestamptz not null default now(),
...
create table if not exists public.prescription_scans (
  ...
  neml_results jsonb not null default '[]'::jsonb,
...
do $$ begin
  create policy "public read drug sources" on public.drug_sources for select using (true);
...
exception when duplicate_object then null; end $$;
do $$ begin
  create policy "public read aliases" on public.drug_aliases for select using (true);
exception when duplicate_object then null; end $$;
do $$ begin
  create policy "public read neml" on public.neml_memberships for select using (true);
exception when duplicate_object then null; end $$;
do $$ begin
  create policy "public read interactions" on public.drug_interactions for select using (true);
exception when duplicate_object then null; end $$;
do $$ begin
  create policy "public read interaction evidence" on public.interaction_evidence for select using (true);
exception when duplicate_object then null; end $$;
do $$ begin
  create policy "users read own scans" on public.prescription_scans for select using (auth.uid() = user_id);
exception when duplicate_object then null; end $$;
do $$ begin
...
returns setof public.neml_memberships
...
returns setof public.interaction_evidence
language sql stable security definer set search_path = public
